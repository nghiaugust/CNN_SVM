...
class CNNFeatureExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        features = self.trained_model.forward_features(x)
        return self.trained_model.forward_head(features, pre_logits=True)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run DeiT-Small CNN or DeiT-Small+SVM inference.")
    parser.add_argument("--config", default=None, help="Deploy config. Default: current folder/config.yaml")
    parser.add_argument("--mode", choices=["cnn", "svm"], default="cnn", help="cnn = classifier head, svm = ViT features + SVM.")
...
def img_size_from_cfg(cfg: dict) -> tuple[int, int]:
    height, width = list(cfg["preprocess"].get("input_size", [128, 512]))
...
def build_model(model_name: str, cfg: dict, num_classes: int) -> nn.Module:
    ...
    return timm.create_model(
        ...
        img_size=img_size_from_cfg(cfg),
    )
...
def load_cnn(checkpoint_path: Path, cfg: dict, num_classes: int, device: torch.device) -> tuple[nn.Module, str]:
    ...
    model = build_model(model_name, cfg, num_classes=num_classes)
